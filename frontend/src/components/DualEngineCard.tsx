...
export default function DualEngineCard({
  ...
  return (
    <section className="card dual-engine-card">
      <div className="dual-engine-head">
        <div>
          <p className="eyebrow">Dual Engine Review</p>
          <h3>Why this review is smarter than a plain LLM answer</h3>
          <p className="muted">
            CodeLens 先用规则引擎抓高确定性风险，再让 AI Reviewer 基于关键上下文做总结和建议。
          </p>
        </div>
      ...
      {analysisTrace.fallback_reason ? (
        <p className="fallback-note">
          当前已回退到规则分析路径，原因: <strong>{analysisTrace.fallback_reason}</strong>。结果仍可用于快速人工复核。
        </p>
      ) : null}
    </section>
  );
